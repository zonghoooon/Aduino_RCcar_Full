...
data = pickle.load( open( "new_additional_data2.p", "rb" ), encoding="latin" )
n_images = len(data)
print(n_images)
...
for direcao,velocidade,img in data:
    ...
    count+= 1
    print(str(count) + " press : ")
    a = int(cv2.waitKey(0))
    a = int(a) -50 # 1 , 2 , 3
    print(int(a))
    with open("new_additional_data_plus2.p", "wb") as f:
        sample = [a,1,img]
        data_pls.append(sample)
